...
\begin{frame}
  ...
  \item<6-> Use \mintinline{Scheme}{(exact? (sqrt 2))} or \mintinline{Scheme}{(inexact? (sqrt 2))}
  %\item<7-> There are many other predicates: \mintinline{Scheme}{real?},
...
\begin{frame}
  ...
  \item<5-> \mintinline{Scheme}{(number->square 10)},  \mintinline{Scheme}{(number->square 5)}, \mintinline{Scheme}{(number->square 1)}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<2-> \mintinline{Scheme}{(big-bang 100 [to-draw number->square])}
...
\begin{frame}
  ...
  \item<5-> So, we will need to use the \mintinline{Scheme}{on-key} event handler.
  \item<6-> But how do we reset the state of the universe?
  \item<7-> Notice that our other handlers like in \mintinline{Scheme}{[on-tick sub1]} take a \emph{function} to create the new universe state.
...
\begin{frame}
  ...
  \begin{itemize}
  \item<2-> \mintinline{Scheme}{(define (reset state key-event) 100)}
...
\begin{frame}
  ...
  \mintinline{Scheme}{(tock (tock (tock cw0)))}
  \pause
  \\\\
  ...
  \mintinline{Scheme}{(define (stop state key) 0)}
\end{frame}
