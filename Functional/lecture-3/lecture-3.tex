...
\begin{frame}
  \frametitle{Handly Libraries to Include}
...
\begin{frame}
  ...
  \item<3-> Evaluate left to right, and then inner parentheses naturally evaluate first: \mintinline{racket}{(+ 1 (+ 1 (+ 1 1) 2) 3 4 5)}
  \item<4-> == \mintinline{racket}{(+ 1 (+ 1 2 2) 3 4 5)}
  \item<5-> == \mintinline{racket}{(+ 1 5 3 4 5)}
  \item<6-> == \mintinline{racket}{18}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item<3-> We can give it multiple arguments: \mintinline[fontsize=\footnotesize]{racket}{(string-append "What a " "lovely " "day" " for Racket!")}
...
\begin{frame}
  ...
  \item<5-> Here is a secret: \mintinline{racket}{(p . ==> . q)}
...
\begin{frame}
  ...
  \item Define the function string-first, which extracts the first 1String from a non-empty string. 
  \item Define the function string-last, which extracts the last 1String from a non-empty string. 
  \item Define the function image-area, which counts the number of pixels in a given image.
...
\begin{frame}
  ...
  \begin{enumerate}
    \setcounter{enumi}{5}
 \item Define the function string-join, which consumes two strings and appends them with "\_" in between.
